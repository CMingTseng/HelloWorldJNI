...
var jniOutputFolder: String? = null      // Output for final libraries, can be null
...
val calculatedJniOutputFolder = jniOutputFolder // The original String? value
project.extra.set("TEST_JNI_OUTPUT_FOLDER", calculatedJniOutputFolder)

project.logger.lifecycle("[jnitask.gradle.kts] Set TEST_JNI_OUTPUT_FOLDER to: '$calculatedJniOutputFolder'")
project.logger.lifecycle("[jnitask.gradle.kts] Project extra properties contains TEST_JNI_OUTPUT_FOLDER: ${project.extra.has("TEST_JNI_OUTPUT_FOLDER")}")

// Also expose the effective path
val effectiveNativeLibPathForRuntime = jniOutputFolder?.let {
    if (it.isNotBlank()) project.file(it).absolutePath else null
} ?: cmakeBuildDirForCurrentOS.absolutePath
project.extra.set("TEST_EFFECTIVE_PATH", effectiveNativeLibPathForRuntime)
project.logger.lifecycle("[jnitask.gradle.kts] Set TEST_EFFECTIVE_PATH to: '$effectiveNativeLibPathForRuntime'")
project.logger.lifecycle("[jnitask.gradle.kts] Project extra properties contains TEST_EFFECTIVE_PATH: ${project.extra.has("TEST_EFFECTIVE_PATH")}")